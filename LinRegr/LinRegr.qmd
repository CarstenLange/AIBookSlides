...
## Custom R Command to Calculate MSE
...
:   **Calculate optimal values for the parameters** $\beta$ based on Ordinary Least Squares (OLS) using two formulas (**Note,** this method works only for linear regression)

**Method 2:**

:   We can use a **systematic trial and error process**.

## Method 1: Calculate Optimal Parameters{.smaller .scollable}
...
## Method 2: Use a Systematic Trial and Error Process to Find Best Parameters 🤓

- **Grid Search (ka Brute force):** For a given range of $\beta_1$ and $\beta_2$ values, build a table with pairs of all combinations of these $\beta a$. The use our custom `mse()` command to calcualte a $MSE$ for each.

- **Optimizer:** Use the R buildin optimizer. Push the data and start values for $\beta_1$ and $\beta_2$ to the optimizer as arguments. The rest is done by the optimizer.
