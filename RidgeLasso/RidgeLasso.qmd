---
title: "Regularization --- Ridge, Lasso, and Elastic Net Penalties"
...
```{webr}
set.seed(777)
ModelDesignLasso=linear_reg(penalty=500, mixture=1) |>  # 0 for Ridge, 1 for Lasso. The argument penalty is the "penalty factor (lambda)"
                 set_engine("glmnet") |> 
                 set_engine("glmnet") |> 
                 set_mode("regression")
...
![Conversion of Parameters $\beta$s for Increasing Penalty Factors($\lambda$)](images/paste-2.png)
:::

::: {.column width="50%"}
**Lasso:**

![Conversion of Parameters $\beta$s for Increasing Penalty Factors($\lambda$)](images/paste-1.png)
:::
:::::
...
$$
T^{arget}_{Elastic}=\underbrace{\frac{1}{20}\sum_{i=1}^{20} \left ( \widehat{Price}_i-Price_i\right)^2}_{MSE}
+ 
\lambda\biggl(\overbrace{
\alpha P^{enalty}_{Lasso}
+
(1-\alpha)P^{enalty}_{Ridge}}^{P^{enalty}_{Elastic}}\biggr)$$
...
+ 
\lambda\biggl(\overbrace{
\alpha\underbrace{ \sum_{i=j}^{5} \rvert \beta_j \rvert}_{P^{enalty}_{Ridge}}
+
(1-\alpha)\underbrace{\sum_{i=j}^{5}  \beta_j^2}_{P^{enalty}_{Lasso}}}^{P^{enalty}_{Elastic}}\biggr)
$$
...
[Exercise Project: Create a Multivariate Polynomial Regression with ElasticNet](https://aibookslides.netlify.app/aibookexercises/07-ridgelassoexerc100){target="_blank"}