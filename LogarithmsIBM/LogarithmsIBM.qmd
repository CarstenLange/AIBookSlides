...
\\ \text{(in MPG)}\\
\text{---------------------------}\\
\text{`r format(summary(Model3)$coefficients[2,1]/100, scientific = FALSE)`} 
\\ \text{(in MPG)}
}}=
`r format(summary(Model3)$coefficients[2,1], scientific = FALSE)`
\underbrace{log(Weight)}_{\substack{\text{Relative Change:}\\
\text{+1} \\ \text{(=100% (doubles)}\\
\text{---------------------------}\\
\text{+0.01}\\
\text{(=1%)}
}} +`r summary(Model3)$coefficients[1,1]`
$$
...
**Model 3: outcome MPG logarithmic and predictor Weight logarithmic ($r^2= `r summary(Model3)$r.squared`$)**
