...
Please complete the code block below. Here are a few hints:
...
`head(DataTestWithPred |> select(Label, .pred_class, everything())[,1:12])` sorts the column `Label`, `.pred_class` to the front of the new data frame `DataTestWithPred` and then outputs the first six augmented observations (columns for `Pxl11` --- `Pxl784` are omitted).
...
2. Sorts the column `Label` and `.pred_class` to the front of the new data frame `DataTestWithPred` and then outputs the first six augmented observations (columns for `Pix9` -- `Pix784` are omitted).
:::
...
#| exercise: ex_6
#| exercise.setup: ex_all
MetricsFctMnist=metric_set(accuracy, sensitivity, specificity)
MetricsFctMnist(DataTestWithPred, truth=______, estimate=______)
```
...
#| exercise: ex_6
#| solution: true
MetricsFctMnist=metric_set(accuracy, sensitivity, specificity) #<1>
MetricsFctMnist(DataTestWithPred, truth=Label, estimate=.pred_class) #<2>
```
