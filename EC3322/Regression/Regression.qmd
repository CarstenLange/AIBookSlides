...
- See what happens if two *Simple Equations* (one predictor each) are combined into one multivariate OLS regression, when predictors are correlated<br>

- Use Virtual Reality to perform the same tasks named in the learning outcomes above with 3D-diagrams<br>
...
```{webr WebRDataHousesLoadShow}
# library(tidyverse) and library(janitor) have been  loaded already
...
```{webr}
#| exercise: exc1
MeanSqft=______(______)
cat("The mean square footage of a house in King county is:", ______)

MeanPrice=______(______)
cat("The mean price of a house in King county is:", ______)
```
...
[![**Click Image to play**](Images/hwsimpleregression.png){width="70%"}](https://horizon.meta.com/world/647544861765620/?locale=en_US){target="_blank"}
...
```{webr}
#| exercise: exc2
ModelOLS=______(______~______, ______=______)
print(ModelOLS)
```
...
ModelLMExerc=______(______~______, ______=______)
print(______)
...
<small>200</small> <br><br> **Price on Condition:**<br>
...
```{webr}
MeanSqft=mean(DataHouseSqftRooms97Cor$Sqft)
cat("The mean square footage in the new dataset is:", MeanSqft)

MeanRooms=mean(DataHouseSqftRooms97Cor$Rooms)
cat("The mean number of rooms in the new dataset is:", MeanRooms)

MeanPrice=mean(DataHouseSqftRooms97Cor$Price)
cat("The mean house price in the dataset is:", MeanPrice)
```
...
[![**Click on image to play on Meta-Quest VR headset**](Images/diagramwithplane.png)](https://vr.lange-analytics.com/diagramexh.html){target="_blank"}
