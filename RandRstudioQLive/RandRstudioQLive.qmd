...
## Extracting the Vectors and Perform Calculations (logical Vectors){.smaller}
...
DataTitanicFinal = mutate(
                   filter(select(DataTitanic, Survived, PasClass=Pclass , 
                                 Sex, Age, FareInPounds), Sex=="female"), 
                   FareIn2021Dollars=164.1*FareInPounds,                   
                   FareInPounds=NULL)
...
In each data frame we would need only the variables `Sex` and `Survived` but we add also `Age` for additional analysis.

## The Male Data Frame

We select `Sex`,  `Survived`, and `Age` and filter for "male":

```{webr}
#| echo: false
DataMale=DataTitanic |> 
         select(Sex,  Survived, Age) |> 
         filter(Sex=="male")
```
...
We select `Sex`,  `Survived`, and `Age` and filter for "female":
...
## Bonus: Comparing the Average Age of Males to Females
