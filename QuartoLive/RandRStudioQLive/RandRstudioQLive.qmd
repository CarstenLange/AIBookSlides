...
## Part 3: Data Types & Data Objects {.smaller}

::: incremental
-   **Data Types:** Which type of values can R store?
    -   numerical `num` (such as: 1, 2.3, 100000)
...
## Part 3: Data Types & Data Objects: Alcoholic Beverages Analogy {.smaller}
...
**Categorical** values are stored as in R as `factor` data type. A `factor` object signals certain R commands that the variable is **categorical rather than character**.\
...
-   **Data Types:** Which type of values can R store? [✓]{style="color:red; font-size:4rem;"}
    -   Numerical
    -   Character
    -   Categorical / Factor
...
DataTitanicSelVar=import("Data/Titanic.csv") |> 
  select(-SiblingsSpousesAboard, -ParentsChildrenAboard)
kable(DataTitanicSelVar[1:12,])
```
...
![Data Type and Object Structure](/Images/DataObjectDataTypes.png){fig-alt="Data Type and Object Structure. Hierachical diagram." fig-align="center"}
...
-   `arrange()` sorts a data frame according to one or more colums in ascending order (use argument `desc()` for descentig order)
...
We now have a data frame with only women and columns $Survived$, $PasClass$, $Sex$, $Age$, and $FareIn2021Dollars$.
...
3.  We mutated to calculate a new variable $FareIn2021Dollars$ and saved finally in `DataTitanicSelVarFemDolFare`
