...
-   How to setup *Positron*
-   How to use a (project) folder in *Positron*
-   How to use major functionality of *Positron*
...
## Install and Setup R and Positron {.smaller}
...
> **Note**<br> - Install R before *Positron*<br> - If an older R version exists deinstall it before installing the newer version

## RStudio --- Integrated Development Environment (IDE/not recommended; outdated) {.smaller}

![RStudio Window](Graphics/RStudioWindowsCL.png)

First steps to setup *R* and *RStudio* can be found in this video: [Click here](https://youtu.be/LmRtTkD5E6M) (However, **it is recommended** to work wit *Positron* rather than *RStudio* in this course)
...
## Set Window Layout and Choose R Compiler {.smaller}

Always open a folder first

![](Graphics/Screenshot%202025-12-11%20003616.png)

## Set Window Layout and Choose R Compiler {.smaller}

dat **Always open or create a folder first**
...
![](Graphics/RStudioKeybindings.png)

## Positron: First Steps with an R file

1.  Print "Hello world" using the `print()` command.

2.  Assign values $3$ and $4$ to the legs $a$ and $b$ of an right-angled triangle.

-   calculate the hyphosenuse $c$ $$c^2=a^2+b^2 \Longleftrightarrow c=\sqrt{a^2+b^2}$$

-   print the result using the `cat(command)`

-   Assign $2$ varaiable ("R" object) `a` and run the `cat()` command again.

## Try Positron with a Quarto (qmd) file {.smaller}

-   Always open a folder first
-   We want to have text and code in the same file.
-   Text is written in *MarkDown*
-   Code is sorundended by:
...
-   Assign $2$ variable ("R" object) `a` and run the `cat()` command again.

## WASC: R Runs in a Browser Includig Libraries and Data
...
For example, to install the `tidyverse` package, type in the consol window: `install.packages("tidyverse)`
...
<br><br> **Data Types:** Which type of values can R store? <br><br> **Data Objects:** What are the containers R uses to store data?
