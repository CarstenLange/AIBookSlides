...
> **Note**<br>
> - Install R before *Positron*<br>
> - If an older R version exists deinstall it before installing the newer version
...
First steps to setup R anf *RStudio* can be found in this video: [Click here](https://youtu.be/LmRtTkD5E6M)
(However, **it is recommended** to work wit *Positron* rather than *RStudio* in this course)
...
## Try Positron with an R file {.smaller}

1. Always open a folder first
1. Print "Hello world" using the `print(command)

3. Assign values $3$ and $4$ to the legs $a$ and $b$ of an right-angled triangle.
  - calculate the hyphosenuse $c$
  - print the result using the `cat(command)`
  $$c^2=a^2+b^2 \longleftrightarrow c=\sqrt{a^2+b^2}$$
