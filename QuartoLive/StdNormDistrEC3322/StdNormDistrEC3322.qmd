...
```{webr}
library(TeachHist)
TeachHistRelFreq(Mean=70, Sd=3, SeedValue=777, NOfSimData=10000)
```
...
```{r}
#| echo: false
TeachHistDens(Mean=70, Sd=3, SeedValue=777, NOfSimData=10000, PlotNormCurv = TRUE)
```
...
## Histogram with Relative Frequencies (Men's Height) {.smaller} 
...
**Try:** What is the probability of having a value of less than $80$ when the mean is $70$ and the standard deviation is $3$.
...
cat("Probability for x<80:",
     pnorm(______,______,______))
pnorm4(______,______,______)
```
:::
::: {.hint exercise="ex_2"}
The `pnorm()` and the `pnorm4()` commands have three arguments, the analyzed value, the mean, and the standard deviation (in that order).
...
To use the table from the Hawkes textbook, you need to know, how many standard deviation $80$ is from the mean because this is the Z-Value (mean=$70$, StdDev=$3$).
...
#| message: false
source("https://econ.lange-analytics.com/RScripts/ZTable4Digits.R")
cat("Probability (rounded) for x < 80:")
...
To calculate how far 80 is from the mean, subtract the mean from 80.
...
#| exercise: ex_3
#| solution: true
DistInchesFromMean= 80 - 70
cat("Distance from Mean in Inches:", DistInchesFromMean)
cat("Distance from Mean in Std. Dev:", DistInchesFromMean/3)
```
...
`pnorm(x, mean=?, sd=?)` calculates the probability of getting a value smaller than x. <br> **Try:** What is the probability of having a value of greater than 80 when the mean is 70 and the standard deviation is 3
...
`qnorm(p, mean=?, sd=?)` `qnorm4(p, mean=?, sd=?)` calculates the value that is not exceeded by x percent of the men<br> **Try:** What is the value that is not exceeded by 95% of men?
...
`qnorm(p, mean=?, sd=?)` and `qnorm4(p, mean=?, sd=?)` calculates the value that is not exceeded by x percent of the men<br> **Try:** Between which values fall 95% of the women (95% confidence interval)
