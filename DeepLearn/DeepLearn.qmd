...
$$MSE= \frac{\sum^N_{i=1}(\widehat P_i - P_i)^2}{N}$$ \begin{eqnarray*}
\widehat{P_i}&=&\beta_7\\
&+&\overbrace{
\frac{1}{1+e^{-(\beta_1 +\beta_2 Carat_i+\beta_3 Clarity_i)}}
}^{A^{ct}_1}\cdot \beta_8
...
Then use the down-arrow to proceed with the slides. Otherwise, use the left-arrow.
...
Go to the AI Book in the link in the footer below and you will find the analysis at the end of the Neural Network chapter.

Alternatively you can run uncommented R-script with the same code here:

[Neural Network Real World Application](https://www.lange-analytics.com/AIBook/Scripts/DeepLearnScript2.R)
