...
-   [How Large Language Models Work by Andreas Schoeffelbauer](https://medium.com/data-science-at-microsoft/how-large-language-models-work-91c362f5b78f)

-   [Super Data Science Course: GenAI & LLMs A-Z](https://community.superdatascience.com/c/llm-gpt)<br> **Strongly recommended**
...
3.  **Context is ignored.** <br>E.g.: <br> `The animal didn't cross the road because it is too tired`<br> or <br> `The animal didn't cross the road because it is too wide`<br> **Solution:** `Attention` (stores for each word which words from the prompt are important and which are not)
:::
...
**2014:** Google Team, "Attention is All you Need", Transformer Model (powerful, fast, and can be parallelized; with small modifications still used in 2025)
...
**Prompt Size:** 0 to 300 words/tokens (32,000 in *ChatGPT*)

**Position Vector Size:** 0 to 16 (512 in *ChatGPT*)

## Step 2: Generate Position-Aware Embedding {.smaller .columns}

::: {.column width="65%"}
#### Example: Position Table with 1, 10, 100 Digits
...
```{r}
#| echo: false
Plot=ggplot(aes(x=WordPosition, y=Ones), size=9, data=filter(TablePos, WordPosition<300))+
...
$i :=$ The Row in the *Position Vector* numbered from $0$ to $16$ (512 in *ChatGPT*)
...
Step 3 identifies for each word in the prompt other words in the prompt that are relevant (**Context**)

Simplified, in **Example 1** the word **it** points to **tired**. In **Example 2** the word **it** points to **wide**.

## Step 3: Adding Context {.smaller}

Using `Position Aware` `Vector X` and a *Neural Net* to create new vectors `q` (what the word searches for), `k-Vector` (what the word has to offer),`v-Vector` (content of Vector X slightly modified).
...
Comparing a words `q-Vector` (what it wants) with all other words `k-Vectors`, including its own, using the **Dot Product**. The results are processed via *SoftMax* for all words and are written in an *importance vector* (see the yello framed vector). It shows the importance of each other word in the prompt for the analyzed word (`It`).
...
The final Position and Context Aware `Vector A` for the word of interest (`it`) is created as a weighted sum from all `v-Vectors` in the prompt.
...
## Step 4 and 5: Multi Head Processing and Residual Connection {.smaller}

**Multi Head Processing:** Use several parts (`Heads`) to add more **Attention**

**Residual Connection:** Add information from previous step (again) to the system.

## Step 6: Training {.smaller}

All words (position and context aware vectors) - `A-Vectors` - from the prompt are simultaneously moved through a non-linear model, and the next word for each word in the prompt is predicted.

![](images/clipboard-480391752.png)asdf
...
## Where to Larn More About Transformers

**GenAI & LLMs A-Z (Advanced Technical Course)** 
<br>
A Super Data Science course. The course is not free but a free trial of Super DataScience is available.
