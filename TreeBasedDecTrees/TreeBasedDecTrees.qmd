...
format: 
  live-revealjs:
    ...
    incremental: false
    scrollable: true
    footer: "<a href='https://ai.lange-analytics.com/htmlbook/TreeBased.html#TreeBased-DecTrees' target='_blank'>Textbook </a> "
   
webr:
  packages: ['dplyr', 'rsample', 'janitor',  'recipes', 'broom',
             'yardstick','parsnip','workflows',rpart, rpart.plot] # Auto-install packages in the browser
...
```{r}
DraftModeCL= FALSE # jitter plots in the last slides take a long time. DraftMode=TRUE skips it. 
```
...
library(kableExtra); library(rpart.plot)
DataTitanic=read.csv("https://ai.lange-analytics.com/data/Titanic.csv") |> 
            clean_names("upper_camel") |>
             select(Survived, Sex, Class=Pclass, Age, Fare=FareInPounds) |>
             mutate(Survived=as.factor(Survived))

set.seed(777)
Split7525=initial_split(DataTitanic, prop=0.75, strata=Survived)
...
DataTitanic=read.csv("https://ai.lange-analytics.com/data/Titanic.csv") |> 
            clean_names(______) |>
            select(Survived, Sex, Class=Pclass,
            Age, Fare=FareInPounds) |>
             mutate(Survived=as.factor(Survived))

set.seed(777)
Split7525=initial_split(______, prop=______, strata =______)
...
RecipeTitanic=recipe(______~______, ______=______)
...
## Variance Reduction Explained (PercRep<`r SplitVal`)
